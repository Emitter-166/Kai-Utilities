    ...
    public void onMessageReceived(MessageReceivedEvent e){
        ...
        if(args[1].equalsIgnoreCase("remove")){
            ...
            if(args[2].equalsIgnoreCase("undo")){
                e.getMessage().reply("`Started undoing`").mentionRepliedUser(false).queue();
                undo_members.forEach(member -> e.getGuild().addRoleToMember(member, undo_role).queue());
                e.getMessage().reply("`Finished undoing`").mentionRepliedUser(false).queue();
                return;
            }
